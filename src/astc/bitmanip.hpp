...
inline void orbits8_ptr(uint8_t* ptr, size_t bitoffset, size_t number, size_t bitcount)
{
  ...
  //
  // FIXME: Writing to the last byte when the number of bytes is a multiple of 2 will write past the allocated memory.
...
{
  public:
    explicit bitwriter(uint8_t* ptr) :
      ptr_(ptr), bitoffset_(0)
    {
